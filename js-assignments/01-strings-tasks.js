...
function encodeToRot13(str) {
    function rot13_map (chars) {
        String.prototype.rotate = function(number) {
        let num = number % this.length;
        let cut = number < 0 ? -number : this.length - number;
        return this.substr(cut) + this.substr(0, cut);
    };
    let rot13_dict = {};
    let alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
    let Az = (alphabet + alphabet.toLowerCase()).split('');
    let Nm = (alphabet.rotate(13) + alphabet.rotate(13).toLowerCase()).split('');
    for (let i = 0; i < Az.length; i++) {
        rot13_dict[Az[i]] = Nm[i];
    }
    if(rot13_dict.hasOwnProperty(chars)){
        return rot13_dict[chars];} else {return chars;}
    }
