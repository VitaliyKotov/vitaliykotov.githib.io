...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
	...
	let str = arrString.toString();
	return str.slice(0, 1) + str.slice(2, 4) + ' ' + str.slice(4, 5) + str.slice(6)
}
...
function isCreditCardNumber(ccn) {
    let splitted = ('' + ccn).split('');
    for(let i = splitted.length - 2; i >= 0; i--) {
        if(Number(splitted[i]) * 2 > 9) {
            splitted[i] = Number(splitted[i]) * 2 - 9;
            i--;  
        } else {
            splitted[i] = Number(splitted[i]) * 2;
            i--;
        }
    }
    ...
    let sum = splitted.reduce((a, b) => a + b);
